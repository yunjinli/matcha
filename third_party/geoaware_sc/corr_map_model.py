# Code from: https://github.com/Junyi42/GeoAware-SC/blob/master/model_utils/corr_map_model.py
...
class Correlation2Displacement(nn.Module):
    def __init__(self, setting=1, window_size=0, final_corr_size=64):
        super(Correlation2Displacement, self).__init__()
        self.setting = setting
        self.window_size = window_size
        self.final_corr_size = final_corr_size  # not used when window_size=0
    ...
    def forward(
        ...
        if self.window_size > 0:  # zero out the corr_map outside the window
            ...
            max_index_x = max_index_flatten % self.final_corr_size  # (H_s * W_s, )
            max_index_y = max_index_flatten // self.final_corr_size  # (H_s * W_s, )
            corr = corr.view(-1, self.final_corr_size, self.final_corr_size)
            window_mask = torch.zeros_like(corr)  # (H_s * W_s, H_t, W_t)
            for i in range(-self.window_size, self.window_size + 1):
                for j in range(-self.window_size, self.window_size + 1):
                    # make sure the index is within the range
                    clamped_y = torch.clamp(
                        max_index_y + i, 0, self.final_corr_size - 1
                    )  # (H_s * W_s, )
                    clamped_x = torch.clamp(
                        max_index_x + j, 0, self.final_corr_size - 1
                    )  # (H_s * W_s, )
        ...
        if self.window_size < 0:  # gaussian kernel
            x = self.apply_gaussian_kernel(
                x.permute(0, 3, 1, 2), sigma=-self.window_size
            )
            x = x.permute(0, 2, 3, 1)
