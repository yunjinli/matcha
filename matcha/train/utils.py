...
def make_batch_online(img, augmentor, difficulty=0.3):
    p1, H1 = augmentor(img, difficulty)
    p2, H2 = augmentor(img, difficulty, TPS=True, prob_deformation=0.7)

    return p1, p2, H1, H2


def get_warped_pts(pts, H, augmentor, h, w):
    warp = augmentor.warp_points_from_raw_batch(H=H, pts=pts)
    warp_mask = (warp[..., 0] >= 0) * (warp[..., 0] < w) * (warp[..., 1] >= 0) * (warp[..., 1] < h)
    warp = torch.cat([warp, warp_mask[..., None].float()], dim=-1)
    return warp
...
def make_batch(augmentor, difficulty=0.3, train=True):
    ...
    with torch.no_grad():  # we dont require grads in the augmentation
        for b in range(augmentor.batch_size):
            ...
            img = (
                torch.tensor(img_list[rdidx], dtype=torch.float32)
                .permute(2, 0, 1)
                .to(augmentor.device)
                .unsqueeze(0)
            )
    ...
    with torch.no_grad():
        ...
        for batch_idx in range(len(p1)):
            with torch.no_grad():
                ...
                mask_valid = (
                        mask1[batch_idx, src_pts[:, 1].long(), src_pts[:, 0].long()]
