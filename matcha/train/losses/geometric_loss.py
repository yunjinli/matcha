...
def info_nce_loss(X, Y, temperature=0.07, eps=1e-8, valid_matches=None, euc=False):
    if X.size() != Y.size() or X.dim() != 2 or Y.dim() != 2:
        ...
        # sim = get_similarities(X, Y, euc) / temperature
    sim = X @ Y.transpose(-2, -1) / temperature
    conf_matrix12 = F.log_softmax(sim, dim=1)
    conf_matrix21 = F.log_softmax(sim.t(), dim=1)
    ...
    loss = -torch.log((positives / sim.sum(dim=-1).sum(dim=-1, keepdim=True)).clip(eps))
    return loss, conf
...
def fine_loss(f1, f2, pts1, pts2, fine_module, ws=7):
    ...
    # pdb.set_trace()
    patches1 = (
        crop_patches(f1.unsqueeze(0), (pts1 + 0.5).long(), size=ws)
        .view(C, N, ws * ws)
        .permute(1, 2, 0)
    )  # [N, ws*ws, C]
    patches2 = (
        crop_patches(f2.unsqueeze(0), (pts2_random + 0.5).long(), size=ws)
        .view(C, N, ws * ws)
        .permute(1, 2, 0)
    )  # [N, ws*ws, C]
    ...
    features = patches1.view(N, ws, ws, C)[:, ws // 2, ws // 2, :].view(
        N, 1, 1, C
    )  # [N, 1, 1, C]
...
def alike_distill_loss(kpts, img, alike_model=None):
    C, H, W = kpts.shape
    kpts = kpts.permute(1, 2, 0)
    img = img.permute(1, 2, 0).expand(-1, -1, 3).cpu().numpy() * 255

    with torch.no_grad():
        alike_kpts = torch.tensor(
            extract_alike_kpts(img, alike_model=alike_model), device=kpts.device
        )
        labels = (
                torch.ones((H, W), dtype=torch.long, device=kpts.device) * 64
        )  # -> Default is non-keypoint (bin 64)
...
def coordinate_classification_loss(coords1, pts1, pts2, conf, ds_scale=8):
    """
    ...
    with torch.no_grad():
        coords1_detached = pts1 * ds_scale

        # find offset
        offsets1_detached = (coords1_detached / ds_scale) - (
                coords1_detached / ds_scale
        ).long()
        offsets1_detached = (offsets1_detached * ds_scale).long()
        labels1 = offsets1_detached[:, 0] + ds_scale * offsets1_detached[:, 1]

    # pdb.set_trace()
    coords1_log = F.log_softmax(coords1, dim=-1)
    # print("coords1_log: ", coords1_log.shape)
