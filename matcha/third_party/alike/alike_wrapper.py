...
ALIKE_PATH = os.path.abspath(os.path.dirname(__file__))
# print(ALIKE_PATH)
...
def extract_alike_kpts(img, alike_model=None):
    if alike_model is not None:
        pred0 = alike_model(img, sub_pixel=True)
    else:
        pred0 = model(img, sub_pixel=True)
    return pred0['keypoints']


def detectAndCompute(img, top_k=4096, alike_model=None):
...
def match_alike(img1, img2, alike_model=None):
    from kornia.feature import DescriptorMatcher
    kornia_matcher = DescriptorMatcher('mnn')
    with torch.inference_mode():
        if alike_model is not None:
            pred0 = alike_model(img1, sub_pixel=True)
            pred1 = alike_model(img2, sub_pixel=True)
        else:
...
def match_alike_customkp(img1, img2, kp_img1, alike_model=None):
    from kornia.feature import DescriptorMatcher
    kornia_matcher = DescriptorMatcher('nn')
    with torch.inference_mode():
        kp_img1 = kp_img1.astype(np.int32)
        if alike_model is not None:
            pred0 = alike_model(img1, sub_pixel=True, return_dense=True)
            pred1 = alike_model(img2, sub_pixel=True, return_dense=True)
        else:
