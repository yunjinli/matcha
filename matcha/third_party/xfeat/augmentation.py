...
def generateRandomTPS(shape, grid=(8, 6), GLOBAL_MULTIPLIER=0.3, prob=0.5):
    ...
    # print("shape: ", shape, sh, sw)
    src = torch.dstack(
        torch.meshgrid(
            torch.arange(0, h + sh, sh)[: grid[0] + 1],
            torch.arange(0, w + sw, sw)[: grid[1] + 1],
            indexing="ij",
        )
    )

    offsets = torch.rand(grid[0] + 1, grid[1] + 1, 2) - 0.5  # [9, 7, 2]
    offsets *= torch.tensor([sh / 2, sw / 2]).view(1, 1, 2) * min(
        0.97, 2.0 * GLOBAL_MULTIPLIER
    )
    # print("src: ", src.shape, offsets.shape)
...
class AugmentationPipe(nn.Module):
    def __init__(
        ...
        self.dims_t = (
            torch.tensor(
                [
                    int(self.dims[0] * (1.0 - self.sides_crop))
                    - int(self.dims[0] * self.sides_crop)
                    - 1,
                    int(self.dims[1] * (1.0 - self.sides_crop))
                    - int(self.dims[1] * self.sides_crop)
                    - 1,
                ]
            )
            .float()
        ...
        self.dims_s = (
            torch.tensor(
                [
                    self.dims_t[0, 0, 0] / out_resolution[0],
                    self.dims_t[0, 0, 1] / out_resolution[1],
                ]
            )
            .float()
            .to(device)
        ...
        list_augmentation = [
            ...
            kornia.augmentation.ColorJitter(0.15, 0.15, 0.15, 0.15, p=1.0),
            kornia.augmentation.RandomEqualize(p=0.4),
            kornia.augmentation.RandomGaussianBlur(
                p=0.3, sigma=(2.0, 2.0), kernel_size=(7, 7)
            ),
        ]
        ...
        if len(self.all_imgs) < 10:
            raise RuntimeError(
                "Couldnt find enough images to train. Please check the path: ", img_dir
            )

        if load_dataset:
            print(
                "[Synthetic] Found a total of ",
                len(self.all_imgs),
                " images for training..",
            )
            ...
            if len(self.all_imgs) - num_test_imgs < max_num_imgs:
                raise RuntimeError(
                    "Error: test set overlaps with training set! Decrease number of test imgs"
                )
    ...
    def load_imgs(self):
        ...
        self.test = [
            ...
            for p in tqdm.tqdm(
                self.all_imgs[-self.num_test_imgs:], desc="loading test"
            )
        ]

    def norm_pts_grid(self, x):
        if len(x.size()) == 2:
            return (x.view(1, -1, 2) * self.dims_s / self.dims_t) * 2.0 - 1
        return (x * self.dims_s / self.dims_t) * 2.0 - 1

    def denorm_pts_grid(self, x):
        if len(x.size()) == 2:
            return ((x.view(1, -1, 2) + 1) / 2.0) / self.dims_s * self.dims_t
        return ((x + 1) / 2.0) / self.dims_s * self.dims_t
    ...
    def warp_points(self, H, pts):
        ...
        offset = torch.tensor(
            [int(self.dims[0] * self.sides_crop), int(self.dims[1] * self.sides_crop)],
            device=pts.device,
        ).float()
    ...
    def forward(self, x, difficulty=0.3, TPS=False, prob_deformation=0.5, test=False):
        ...
        with torch.no_grad():
            ...
            H = torch.tensor(
                np.array(
                    [
                        generateRandomHomography(shape, difficulty)
            ...
            output = kornia.geometry.transform.warp_perspective(
                x, H, dsize=shape, padding_mode="zeros"
            )
            ...
            high_h = int(h * (1.0 - self.sides_crop))
            high_w = int(w * (1.0 - self.sides_crop))
            ...
            if TPS:
                ...
                for b in range(self.batch_size):
                    b_src, b_weights, b_A = generateRandomTPS(
                        shape, (8, 6), difficulty, prob=prob_deformation
                    )
                    b_src, b_weights, b_A = (
                        b_src.to(self.device),
                        b_weights.to(self.device),
                        b_A.to(self.device),
                    )
            ...
            if np.random.uniform() > 0.5 and self.photometric:
                noise = F.interpolate(
                    torch.randn_like(output) * (10 / 255), (h // 2, w // 2)
                )
                noise = F.interpolate(noise, (h, w), mode="bicubic")
                output = torch.clip(output + noise, 0.0, 1.0)
    ...
    def get_correspondences(self, kps_target, T):
        ...
        undeformed = self.denorm_pts_grid(
            warp_points_tps(self.norm_pts_grid(kps_target), src, W, A)
        ).view(-1, 2)
...
class OnlineAugmentorPipe(nn.Module):
    def __init__(self,
                 device,
                 warp_resolution=(1200, 900),
                 out_resolution=(400, 300),
        ...
        super(OnlineAugmentorPipe, self).__init__()
        self.half = 16
        self.device = device
    ...
    def warp_points_from_raw(self, H, pts):
        scale = self.dims_s.view(-1, 2)
        offset = torch.tensor(
            [int(self.dims[0] * self.sides_crop), int(self.dims[1] * self.sides_crop)],
            device=pts.device,
        ).float()
        # pts = pts * scale + offset # recover to  original size
        pts = torch.vstack([pts.t(), torch.ones(1, pts.shape[0], device=pts.device)])
        warped = torch.matmul(H, pts)
        warped = warped / warped[2, ...]
        warped = warped.t()[:, :2]
    ...
    def warp_points_from_raw_batch(self, H, pts):
        scale = self.dims_s.view(-1, 2)
        offset = torch.tensor(
            [int(self.dims[0] * self.sides_crop), int(self.dims[1] * self.sides_crop)],
            device=pts.device,
        ...
        # pts = torch.vstack([pts.t(), torch.ones(1, pts.shape[0], device=pts.device)])
        pts = torch.cat([pts, torch.ones(size=(pts.shape[0], pts.shape[1], 1), device=pts.device)], dim=-1)  # [B, N, 3]
        # warped = torch.matmul(H, pts)
        warped = torch.bmm(H, pts.permute(0, 2, 1)).permute(0, 2, 1)  # [B, N, 3]
        warped = warped / warped[:, :, 2:]
        # print('warped: ', warped)
        warped = warped[:, :, :2]
        # return warped
